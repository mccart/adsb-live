...
const countries = sd.prepare(`SELECT name FROM Country ORDER BY name`);
countries.pluck();

const codeBlocks = sd.prepare(
...
function toQuery({ query, options = {} }) {
  ...
  if (query) {
    console.log("Generating where...");
    let where = toWhere(query);
    sql += where[0];
    values = where[1];
  } else console.log("CANNOT GENERATE WHERE");
...
const Api = {
  aircraft: query => {
    ...
    console.log("Parsed", terms, values);
